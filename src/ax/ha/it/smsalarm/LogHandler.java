...
public class LogHandler {
	...
	private LogHandler() {
		// Log information
		this.logCatTxt(this.INFO, this.LOG_TAG + ":LogHandler()",
				"New instance of LogHandler created");
	}
	...
	public void logCat(int priority, String logTag, String message) {
		...
		default:
			...
			this.logCat(this.ERROR, this.LOG_TAG + ":logCat()",
					"Log priority wasn't found, check that correct priority constant is used."
							+ " Original logmessage came from: " + logTag
	...
	public void logCat(int priority, String logTag, String message,
		...
		default:
			...
			this.logCat(this.ERROR, this.LOG_TAG + ":logCat()",
					"Log priority wasn't found, check that correct priority constant is used."
							+ " Original logmessage came from: " + logTag
							+ ", with message: " + message + " and throwable: "
							+ thr);
		}
	}
	...
	public void logTxt(int priority, String logTag, String message) {
		...
		// creating continue
		if (this.mExternalStorageAvailable == true
			...
			try {
				...
				case (ASSERT):// <-- 7
					...
					break;
				case (DEBUG):// <-- 3
					bW.write("DEBUG\t\t" + this.formatter.format(this.today)
							+ "\t" + logTag + "#" + message + this.EOL);
					bW.newLine();
					break;
				case (ERROR):// <-- 6
					bW.write("ERROR\t\t" + this.formatter.format(this.today)
							+ "\t" + logTag + "#" + message + this.EOL);
					bW.newLine();
					break;
				case (INFO):// <-- 4
					bW.write("INFO\t\t" + this.formatter.format(this.today)
							+ "\t" + logTag + "#" + message + this.EOL);
					bW.newLine();
					break;
				case (VERBOSE):// <-- 2
					bW.write("VERBOSE\t\t" + this.formatter.format(this.today)
							+ "\t" + logTag + "#" + message + this.EOL);
					bW.newLine();
					break;
				case (WARN):// <-- 5
					bW.write("WARN\t\t" + this.formatter.format(this.today)
							+ "\t" + logTag + "#" + message + this.EOL);
					bW.newLine();
					break;
				default:
					...
					this.logCat(
							this.ERROR,
							this.LOG_TAG + ":logTxt()",
							"Can't write to file because log priority wasn't found, check that correct priority constant is used");
				}
				...
				e.printStackTrace();
				this.logCat(this.ERROR, this.LOG_TAG + ":logTxt()",
						"An Exception occurred during writing to file: "
								+ this.TXT_LOG_FILE, e);
			}
		} else {
			...
			this.logCat(
					this.WARN,
					this.LOG_TAG + ":logTxt()",
					"Cannot log to file because external storage isn't available, check earlier logs fore more details");
		}
	}
	...
	/**
	 * Method to check if directory needed directory or file for application exist,
	 * if not theyr'e created.
	 * 
	 * @param saDir Path and name of directory as File
	 * @param saFile Path and name of file as File
	 * @param file File object used to create a new file or directory
	 * 
	...
	private void checkDirAndFile(File saDir, File saFile, File file) {
		...
		if (!saDir.exists()) {
			...
			try {
				...
				e.printStackTrace();
				this.logCat(this.ERROR, this.LOG_TAG + ":logTxt()",
						"An IOException occurred during creation of file: "
								+ this.TXT_LOG_FILE, e);
			}
	...
	private void writeLogHeader(File file) {
		try {
			// Create BufferedWriter with FileWriter of file, used to
			// write to file
			BufferedWriter bW = new BufferedWriter(new FileWriter(file,
					false));
			...
			// Ensure that everything has been written to the file and
			// close
			bW.flush();
			bW.close();
		} catch (Exception e) {
...
//			this.logCatTxt(this.INFO, this.LOG_TAG
//					+ ":checkExternalStorageState()",
//					"External storage is available for both reading and writing");
		} else if (Environment.MEDIA_MOUNTED_READ_ONLY.equals(this.STATE)) {
			// We can only read the media
			this.mExternalStorageAvailable = true;
			this.mExternalStorageWriteable = false;
			this.logCatTxt(this.WARN, this.LOG_TAG
					+ ":checkExternalStorageState()",
					"Cannot write log, cause: External storage is not available for writing");
		} else {
			...
			this.logCatTxt(
					this.ERROR,
					this.LOG_TAG + ":checkExternalStorageState()",
					"Cannot write log, cause: External storage is not available for writing or reading");
		}
	}
}
