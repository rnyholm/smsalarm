...
public class PreferencesHandler {
	...
	private PreferencesHandler() {
		...
		// Log information
		logger.logCatTxt(logger.getINFO(), this.LOG_TAG
				+ ":PreferencesHandler()",
				"New instance of PreferencesHandler created");
	}
	...
	public Object getPrefs(String sharedPreferences, String key, int type,
		...
		case (INTEGER): // <-- 0
			this.logger.logCatTxt(
					this.logger.getDEBUG(),
					this.LOG_TAG + ":getPrefs()",
					"Integer with value \""
							+ Integer.toString(sharedPref.getInt(key, 0))
		...
		case (STRING): // <-- 1
			this.logger.logCatTxt(this.logger.getDEBUG(), this.LOG_TAG
					+ ":getPrefs()",
					"String with value \"" + sharedPref.getString(key, "")
							+ "\" retrieved from shared preferences: \""
							+ sharedPreferences + "\", with key: \"" + key
							+ "\", type: \"" + Integer.toString(type)
							+ "\" and context: \"" + context.toString() + "\"");
		...
		case (BOOLEAN): // <-- 2
			this.logger.logCatTxt(this.logger.getDEBUG(), this.LOG_TAG
					+ ":getPrefs()",
					"Boolean with value \"" + sharedPref.getBoolean(key, false)
							+ "\" retrieved from shared preferences: \""
							+ sharedPreferences + "\", with key: \"" + key
							+ "\", type: \"" + Integer.toString(type)
							+ "\" and context: \"" + context.toString() + "\"");
	...
	public void setPrefs(String sharedPreferences, String key, Object object,
			Context context) {
		// Set shared preferences from context
		this.sharedPref = context.getSharedPreferences(sharedPreferences,
				Context.MODE_PRIVATE);
		...
		if (object instanceof Integer) {
			...
			this.logger.logCatTxt(this.logger.getDEBUG(), this.LOG_TAG
					+ ":setPrefs()",
					"Instance of Integer stored to shared preferences: \""
							+ sharedPreferences + "\", with key: \"" + key
			...
			this.logger.logCatTxt(this.logger.getDEBUG(), this.LOG_TAG
					+ ":setPrefs()",
					"Instance of String stored to shared preferences: \""
							+ sharedPreferences + "\", with key: \"" + key
			...
			this.logger.logCatTxt(this.logger.getDEBUG(), this.LOG_TAG
					+ ":setPrefs()",
					"Instance of Boolean stored to shared preferences: \""
							+ sharedPreferences + "\", with key: \"" + key
							+ "\", value: \"" + (Boolean) object + "\" and context: \""
							+ context.toString() + "\"");
			...
			// Use JSON to serialize arraylist containing the secondary alarm
			// numbers to a string
			JSONArray a = new JSONArray();

			// Iterate through each element in list and add object in JSON
			// object
			for (int i = 0; i < list.size(); i++) {
				a.put(list.get(i));
			}

			// If list is not empty add it to shared preferences, if empty add
			...
			if (!list.isEmpty()) {
				...
				this.logger.logCatTxt(this.logger.getDEBUG(), this.LOG_TAG
						+ ":setPrefs()",
						"Instance of List<String> stored as String to shared preferences: \""
								+ sharedPreferences + "\", with key: \"" + key
								+ "\", value: \"" + a.toString() + "\" and context: \""
								+ context.toString() + "\"");
			} else {
			...
			this.logger.logCatTxt(this.logger.getWARN(), this.LOG_TAG
					+ ":setPrefs()",
					"Failed to store data to shared preferences: \""
							+ sharedPreferences + "\", with key: \"" + key
							+ "\" and context: \"" + context.toString()
							+ "\". Cause Unsupported datatype");
		}
	}
}
